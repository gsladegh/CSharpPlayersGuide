...
// start with an open box
Box thebox = Box.Open;

do
{
    DisplayCurrentBoxState();
    string command = AskForBoxCommand();
    ChangeBoxState(command);    
} while (true);


void ChangeBoxState(string command)
{
    if (command == "lock" && thebox == Box.Closed)
    {
        thebox = Box.Locked;
    }
    else if (command == "unlock" && thebox == Box.Locked)
    {
        thebox = Box.Closed;
    }
    else if (command == "open" && thebox == Box.Closed)
    {
        thebox = Box.Open;
    }
    else if (command == "close" && thebox == Box.Open)
    {
        thebox = Box.Closed;
    }
}

string AskForBoxCommand()
{
    Console.Write("What do you want to do? ");
    return Console.ReadLine();
}

void DisplayCurrentBoxState()
{
    Console.Write($"The chest is {thebox}. ");
}
